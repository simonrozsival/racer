...
struct benchmark_result {
  ...
  benchmark_result(search_result result,
                   std::chrono::milliseconds computation_time,
                   bool exceeded_time_limit)
      : result{result},
        time_to_last_but_one_waypoint{
            find_time_to_last_but_one_waypoint(result)},
        computation_time{computation_time}, exceeded_time_limit{
                                                exceeded_time_limit} {}
...
void print_result(const std::string &algorithm, const std::string &circuit_name,
                  const std::size_t number_of_actions, const std::size_t from,
  ...
  std::cout
      << algorithm << ", " << circuit_name << ", " << number_of_actions << ", "
      << from << ", " << lookahead << ", "
      ...
      << measurement_sample.result.expanded_nodes.size() << ", "
      << (measurement_sample.result.was_successful()
              ? measurement_sample.result.final_cost
              : 0)
      << ", "
      << ", "
      << (measurement_sample.result.was_successful()
              ? measurement_sample.result.found_trajectory.total_distance()
              : 0)
      << ", " << time_step_s << ", " << discretization << ", " << success_rate
      << ", " << number_of_repetitions << ", " << average_computation_time
      << ", " << computation_time_variance << std::endl;
}

void print_unsuccessful_result(
    const std::string &algorithm, const std::string &circuit_name,
    const std::size_t number_of_actions, const std::size_t from,
    const std::size_t lookahead, const double time_step_s,
    const std::string discretization,
    const benchmark_result &measurement_sample,
    const std::size_t number_of_repetitions, const double time_limit) {
  std::cout << algorithm << ", " << circuit_name << ", " << number_of_actions
            << ", " << from << ", " << lookahead << ", "
            << (!measurement_sample.exceeded_time_limit ? "yes" : "no") << ", "
            << "no"
            << ", " << measurement_sample.result.number_of_opened_nodes << ", "
            << measurement_sample.result.expanded_nodes.size() << ", "
            << "0, "
            << "0, " << time_step_s << ", " << discretization << ", "
            << "0, " << number_of_repetitions << ", "
            << measurement_sample.computation_time.count() << ", "
            << "0" << std::endl;
}

std::string experiment_name(const std::string algorithm,
                            const std::string circuit_name,
                            const std::size_t number_of_actions,
                            const std::size_t from, const std::size_t lookahead,
                            const double time_step_s,
                            const std::string discretization) {
  std::stringstream file_name;

  file_name << algorithm << "_" << circuit_name << "_"
