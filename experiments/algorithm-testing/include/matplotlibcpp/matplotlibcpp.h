...
template <typename Numeric>
bool plot(const std::vector<Numeric> &x, const std::vector<Numeric> &y, const std::string &s, const std::map<std::string, std::string> &keywords)
{
    ...
    {
        if (it->first == "linewidth" || it->first == "markersize")
...
{
    return plot(x, y, std::string(""), keywords);
}
