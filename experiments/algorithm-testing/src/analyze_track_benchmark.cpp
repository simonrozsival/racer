...
int main(int argc, char *argv[]) {
  std::cout
      << "Track analysis BENCHMARK standalone experiment - The Racer project"
      << std::endl
      << std::endl;

  if (argc == 1) {
    std::cerr << "At least one argument is requried - path of a circuit "
                 "definition file."
              << std::endl;
    return 1;
  }
  ...
  for (int i = 1; i < argc; ++i) {
    ...
    const std::shared_ptr<track_analysis_input> config =
        track_analysis_input::load(input_file_name);
    if (!config) {
  ...
  const auto vehicle = racer::vehicle_model::vehicle_chassis::rc_beast();

  // Step 1
  std::cout << "1. Space exploration" << std::endl;
  std::cout << "--------------------" << std::endl;
  std::cout << "circuit name, neighbor circles, number of circles, path "
               "length, computation time in ms"
            << std::endl;
  ...
  for (std::size_t i = 0; i < configs.size(); ++i) {
    ...
    for (int neighbor_circles = 4; neighbor_circles < 16; ++neighbor_circles) {
      ...
      for (std::size_t i = 0; i < repetitions; ++i) {
        ...
        racer::sehs::space_exploration se(vehicle->radius(),
                                          5 * vehicle->radius(), 5);
        const auto circles =
            se.explore_grid(config->occupancy_grid, config->initial_position,
                            config->checkpoints);
      ...
      std::cout << config->name << ", " << neighbor_circles << ", "
                << path.size() << ", " << path_length << ", "
                << (std::accumulate(ms.begin(), ms.end(), 0) / ms.size())
                << std::endl;
    }
  }
  ...
  std::cout << "--------------------" << std::endl;
  std::cout << "circuit name, number of pivot points, computation time in ms"
            << std::endl;
  ...
  for (std::size_t i = 0; i < configs.size(); ++i) {
    ...
    for (std::size_t j = 0; j < repetitions; ++j) {
      ...
      const auto pivot_points = analysis.find_pivot_points(
          paths_of_circles[i], config->checkpoints, config->occupancy_grid);
    ...
    std::cout << config->name << ", " << pivot_points_outputs[i].size() << ", "
              << (std::accumulate(ms.begin(), ms.end(), 0) / ms.size())
              << std::endl;
  }
