...
int main(int argc, char *argv[]) {
  ...
  racer::sehs::space_exploration se(vehicle->radius(), 5.0 * vehicle->radius(),
                                    config->neighbor_circles);
  const auto circles = se.explore_grid(
      config->occupancy_grid, config->initial_position, config->checkpoints);
  stop_stopwatch("space exploration", se_start);

  if (circles.empty()) {
    std::cout << "Space exploration failed, cannot proceed to track analysis."
              << std::endl;
    return 3;
  }
  ...
  const auto raw_waypoints = analysis.find_pivot_points(
      circles, config->checkpoints, config->occupancy_grid);
