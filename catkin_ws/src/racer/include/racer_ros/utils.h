...
msg_to_grid(const nav_msgs::OccupancyGrid &map) {
  ...
  racer::math::vector origin{map.info.origin.position.x,
...
racer::vehicle::trajectory<racer::vehicle::kinematic::state>
msg_to_trajectory(const racer_msgs::Trajectory &msg, double time_step_s) {
  std::vector<racer::vehicle::trajectory_step<racer::vehicle::kinematic::state>>
      steps;
std:
  size_t t = 0;
  for (const auto &step : msg.trajectory) {
    racer::vehicle::configuration configuration(
        {step.pose.position.x, step.pose.position.y},
        tf::getYaw(step.pose.orientation));
    const auto motor_rpm = step.motor_rpm.data;
    racer::vehicle::kinematic::state state{configuration, motor_rpm, 0};

    steps.emplace_back(state, racer::vehicle::action(0, 0),
                       step.next_waypoint.data, t++ * time_step_s);
  }

  return {steps, time_step_s};
}

racer_msgs::Trajectory trajectory_to_msg(
    racer::vehicle::trajectory<racer::vehicle::kinematic::state> trajectory,
  ...
  for (const auto &step : trajectory.steps()) {
    ...
    msg.trajectory.push_back(state);
  }

  return msg;
}

racer_msgs::State state_to_msg(const racer::vehicle::kinematic::state &state,
...
action_to_ackermann_msg(const racer::vehicle::action &action,
...
racer::vehicle::action
twist_to_action(const geometry_msgs::Twist::ConstPtr &msg) {
