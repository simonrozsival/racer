...
struct point
{
  ...
  point(racer::math::point grid_coord, double speed) : coordinate{ grid_coord }, maximum_speed{ speed }
  {
  }
};
...
struct corner
{
  ...
  corner(point t, point a, point e) : turn_in{ t }, apex{ a }, exit{ e }
  {
  }
};
...
{
public:
  static std::unique_ptr<racing_line> basic_from_apexes(std::shared_ptr<racer::occupancy_grid> grid,
                                                        ...
                                                        double max_speed)
  {
    ...
    for (std::size_t i = 0; i < corner_locations.size(); ++i)
    {
      ...
      const auto turn_in =
          (-1.0 * tangent).interpolate_with(to_prev.normalized(), 0.5, 0.5) * (to_prev.length() * 0.3) + apex;
      const auto exit = tangent.interpolate_with(to_next.normalized(), 0.5, 0.5) * (to_next.length() * 0.3) + apex;

      // the smaller (more acute) the angle at the apex, the slower the car should go
      ...
      corners.emplace_back(point{ turn_in, slow_turn_in ? std::min(0.6 * max_speed, 3 * min_speed) : max_speed },
                           point{ apex, min_speed }, point{ exit, max_speed });
    }

    return std::make_unique<racing_line>(corners);
  }

  racing_line(std::vector<corner> corners) : corners_{ corners }, spline_{ enumerate_spline(corners) }
  {
  }

  racing_line(std::vector<corner> corners, std::vector<point> spline) : corners_{ corners }, spline_{ spline }
  {
  }
  ...
  std::vector<point> enumerate_spline(std::vector<corner> corners) const
  {
    ...
    for (std::size_t b{ 0 }; b < control_points.size(); ++b)
    {
      ...
      const auto next_segment = racer::splines::catmull_rom::enumerate_segment(
          control_points[a], control_points[b], control_points[c], control_points[d], step);
