...
public:
  search_problem(
      ...
      const std::shared_ptr<base_discretization<DiscreteState, State>>
          discretize,
...
public:
  ...
  const racer::vehicle::trajectory<State> reconstruct_trajectory(
      const search_node<DiscreteState, State> &node) const override {
...
private:
  ...
  const std::shared_ptr<racer::vehicle::base_vehicle_model<State>>
      vehicle_model_;
...
private:
  double
  prepend_states(std::vector<racer::vehicle::trajectory_step<State>> &path,
