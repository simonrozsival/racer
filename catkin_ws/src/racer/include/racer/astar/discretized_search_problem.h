...
public:
  ...
  valid_neighbors(const search_node<DiscreteState, State> &node) const override
  {
  ...
  {
    auto dist = circuit_->distance_to_waypoint(examined_state.position(), passed_waypoints) +
...
private:
  double prepend_states(std::vector<trajectory_step<State>> &path, const search_node<DiscreteState, State> &node,
