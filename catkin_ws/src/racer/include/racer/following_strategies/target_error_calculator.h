...
public:
  ...
  double calculate_error(
      const std::vector<State> &prediction,
      const racer::vehicle::trajectory<State> &reference,
...
private:
  ...
  double obstacle_proximity_error(
    ...
    const double head_on_dist = std::min(
        grid->find_distance(a.position(), a.cfg().heading_angle(),
                            head_on_clearance),
        std::min(grid->find_distance(a.position(),
                                     a.cfg().heading_angle() +
                                         racer::math::angle::from_degrees(7),
                                     head_on_clearance),
                 grid->find_distance(a.position(),
                                     a.cfg().heading_angle() -
                                         racer::math::angle::from_degrees(7),
                                     head_on_clearance)));
