...
private:
  ...
  std::vector<racer::vehicle::action> available_actions_;
  ...
  select_action(const State &current_state, const std::size_t passed_waypoints,
                const racer::vehicle::trajectory<State> &reference_trajectory,
                const std::shared_ptr<racer::track::occupancy_grid> map,
                bool unsafe) const {
    ...
    for (const auto next_action : available_actions_) {
      ...
      if (!trajectory.empty()) {
        const double error =
            target_error_calculator_.calculate_error(trajectory, should_follow,
                                                     map) +
