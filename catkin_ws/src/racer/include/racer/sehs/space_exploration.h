...
public:
  space_exploration(const double min_radius, const double max_radius,
  ...
  const std::vector<racer::math::circle>
  explore_grid(const std::shared_ptr<racer::track::occupancy_grid> grid,
    ...
    while (goal_it != waypoints.cend()) {
      auto to_next_waypoint =
          find_path(path.back(), initial_heading_angle, *goal_it, grid);
...
private:
  ...
  racer::math::circle generate_circle_around(
      racer::math::point point,
  ...
  const std::vector<racer::math::circle>
  find_path(const racer::math::circle &from, double initial_heading_angle,
            const racer::math::point &to,
            const std::shared_ptr<racer::track::occupancy_grid> grid) const {
    std::priority_queue<std::shared_ptr<circle_node>,
                        std::vector<std::shared_ptr<circle_node>>,
                        distance_estimate>
        open{distance_estimate()};
    std::vector<racer::math::circle> closed;

    open.push(std::make_shared<circle_node>(from, 0, 0, initial_heading_angle,
                                            nullptr));
    ...
    while (open.size() > 0 && closed.size() < 100000) {
      ...
      for (auto node : expand(nearest, to, grid)) {
        if (std::find(closed.begin(), closed.end(), node->examined_circle) ==
            closed.end()) {
          open.push(node);
        }
      }
    ...
    std::cerr << "space exploration failed after exploring " << closed.size()
              << " nodes between [" << from.center().x() << ", "
              << from.center().y() << "] and [" << to.x() << ", " << to.y()
              << "]" << std::endl;
  ...
  std::vector<std::shared_ptr<circle_node>>
  expand(const std::shared_ptr<circle_node> &node,
         const racer::math::point &goal,
         const std::shared_ptr<racer::track::occupancy_grid> grid) const {
    std::vector<std::shared_ptr<circle_node>> nodes;
    const double sector_angle =
        2 * M_PI; // this could be changed to narrow the search direction

    for (auto center : node->examined_circle.points_on_circumference(
             node->heading_angle - (sector_angle / 2), sector_angle,
             number_of_expanded_points_)) {
      ...
      if (circle.radius() >= min_radius_) {
        ...
        nodes.push_back(std::make_shared<circle_node>(
            circle, node->distance_from_start + node->examined_circle.radius(),
    ...
    do {
      ...
      for (std::size_t i = 1; i < can_be_optimized.size() - 1; ++i) {
        if (can_be_optimized[i]) {
          const auto optimized_circle =
              try_to_optimize(circles[i - 1], circles[i], circles[i + 1], grid);
          if (optimized_circle.center() != circles[i].center() &&
              optimized_circle.radius() != circles[i].radius()) {
  ...
  racer::math::circle try_to_optimize(
      const racer::math::circle &a, const racer::math::circle &b,
      const racer::math::circle &c,
      const std::shared_ptr<racer::track::occupancy_grid> grid) const {
    racer::math::point interpolated_center =
        a.center().interpolate_with(c.center(), a.radius(), b.radius());
    const double radius =
        grid->distance_to_closest_obstacle(interpolated_center, max_radius_);
    return radius >= b.radius()
               ? racer::math::circle(interpolated_center, radius)
               : b;
  }
