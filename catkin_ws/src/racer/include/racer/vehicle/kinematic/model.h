...
public:
  state predict_next_state(const state &current, const action &input,
    ...
    configuration translation_and_rotation{v * cos(theta + beta) * dt,
                                           v * sin(theta + beta) * dt,
                                           v * cos(beta) * tan(delta) / L * dt};

    return {current.cfg() + translation_and_rotation, motor_rpm,
            steering_angle};
  }

  double maximum_theoretical_speed() const override {
    const double motor_revolutions_per_second =
        chassis->motor->max_rpm() / 60.0;
    const double wheel_revolutions_per_second =
        motor_revolutions_per_second / chassis->motor_to_wheel_gear_ratio;
  ...
  action action_from_speed_and_steering_angle(const double speed,
    ...
    return {std::clamp(speed / top_speed_, 0.0, 1.0),
            chassis->steering_servo->action_input_for_angle(angle)};
  }

  double top_speed() const { return top_speed_; }
