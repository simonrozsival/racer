...
namespace racer::vehicle_model
{
  ...
  public:
    steering_servo_model(racer::math::angle max_steering_angle_left,
                         ...
                         std::array<double, 2> angle_to_signal,
                         std::array<double, 2> signal_adjustment_coefficients)
    ...
    {
      return std::make_unique<steering_servo_model>(
          angle::from_degrees(10.5), angle::from_degrees(-10.5),
          std::array<double, 2>{12.701, 1476.686},
          std::array<double, 2>{0.0006, 0.1});
    }
  ...
  private:
    ...
    inline double time_to_adjust(const angle alpha, const angle beta) const
    {
      ...
      return signal_adjustment_coefficients_[0] * signal_distance +
             signal_adjustment_coefficients_[1];
    }
  };
