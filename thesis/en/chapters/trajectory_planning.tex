...
\nicefrac{\Delta \delta}{t(\delta, \delta_t)} & \text{when }\delta \neq \delta_t\\
0 & \text{otherwise.}
\end{cases}
\end{aligned}
\end{equation}
...
From the slip angle, we can calculate the lateral force acting on each of the virtual wheels based on the slip angle curve. We approximate this curve with a simple linear model shown in Figure~\ref{fig:lateral_slip_angle_curve} and expressed by the equation:
...
\begin{figure}
	\centering
	\missingfigure{Lateral slip curve}
	\label{fig:lateral_slip_angle_curve}
\end{figure}
