...
Space exploration is a pre-processing step of the planning algorithm. We are looking for the shortest path of overlapping circles in $\mathbb{R}^2$ form the initial state position to the goal location. The centers of each circle lies on the circumference the previous circle along the path. The radii of the circles are in some interval $(r_{min}, r_{max})\subset \mathbb{R}_{>0}$ and each radius is chosen so that it is as large as possible, but none of the circles can overlap a cell with an obstacle in the occupancy grid map of the track. The path of circles does not give us only the shortest path from the initial state to a goal.
...
\begin{algorithm}[]
	...
	\KwIn{Occupancy grid $O_r^{m\times n}$, initial position $\vec{x_0}\in \mathbb{R}^2$, list of waypoints $\hat{w}=\left\langle \vec{w_1}, \vec{w_2}, \ldots, \vec{w_l} \right\rangle\in\mathbb{R}^l$.}
	\KwOut{Path of circles or $null$}
	\Parameter{Range of valid radii $\left(r_{min}, r_{max}\right)\subset\mathbb{R}_{>0}$, number of child circles $n\in \mathbb{N}$}
...
We will implement this process with a three step algorithm which will identify the corners and points along long winding bends:
\begin{itemize}
	\item The first step will be to find the shortest path through the grid which starts at the initial position of the vehicle and which goes through the checkpoints in the correct order and at any point it does not come closer to an obstacle than to a distance of the safety radius. 
	\item The second step will simply traverse the path once and select a sub-sequence of the points such that for two consecutive points $A$ and $B$, $B$ was the last point of the points immediately following $A$ on the original track which are directly visible from $A$. 
	\item In the third step, we will merge points, which are close together (e.g., in a 180° hairpin turn).
\end{itemize}

The shortest path can be found in several different ways. We can reuse the \textit{Space Exploration} algorithm which was described in Section~\ref{sec:space_exploration}. The centers of the path of circles found by the \textit{SE} algorithm will never be too close to the boundary of the track, which acts as the imaginary ``rubber wall'' form our thought experiment. We can now proceed the second step of finding the corners of the circuit. We will traverse the path and mark the centers of circles which are the last directly visible points from the previous point as shown in algorithm~\ref{alg:find_apexes}. We will re-use the direct visibility relation $D$ in an occupancy grid $O_r^{m\times n}$ given in Definition~\ref{def:directly_visible_points}.
...
\begin{algorithm}[H]
	...
	\SetKwFunction{FindApexes}{FindApexes}
	\SetKwFunction{SpaceExploration}{SpaceExploration}
	
	\KwIn{Occupancy grid $O_r^{m\times n}$, initial configuration $c_0$, directly visible points relation $D$, sequence of checkpoints $\hat{p}_n=\left\langle \vec{p_0}, \vec{p_1}, \ldots, \vec{p_n} \right\rangle$}
	\KwOut{Sequence of waypoints}
		
	\SetKwProg{algorithm}{Algorithm}{:}{}
		
	\BlankLine
	
	\algorithm{\FindApexes{$c_0$, $\hat{p}_n$, $O_r^{m\times n}$}}{
		$\hat{q}_m\gets $ \SpaceExploration{$c_0$, $\hat{p}_n$, $O_r^{m\times n}$}\;
		$\hat{w}\gets \left\langle \right\rangle$\;
		$w_{last}\gets q_0, q_{prev}\gets q_0$\;
		\ForEach{$q_{next}\in \hat{q}_m$} {
			\If{$(w_{last}, q_{next})\notin D$}{
				$\hat{w}\gets \hat{w}+\left\langle q_{prev} \right\rangle$\;
				$w_{last}\gets q_{prev}$\;
			}
			$q_{prev}\gets q_{next}$\;
		}
		\ForEach{$q_{next}\in \hat{q}_m$} {
			\If{$(w_{last}, q_{next})\notin D$}{
				$\hat{w}\gets \hat{w}+\left\langle q_{prev} \right\rangle$\;
				$w_{last}\gets q_{prev}$\;
				\If{$(w_{last}, w_{0})\in D$}{
					\KwRet{$\hat{w}$};
				}
			}
			$q_{prev}\gets q_{next}$\;
