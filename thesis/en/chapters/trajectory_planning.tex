...
This function used to output the next state $x'$ after an action $u$ is applied to a state $x$, i.e. $x'=f(x, u)$. When the state space is continuous, the outcome of an execution of an action depends on for how long it is being applied. Instead of a state transition, the differential constraints now express the velocity in the configuration space:

\[
	\dot{x}=f(x, u).
\]

To predict, how the state at some given point in time $x(t)\in X$ changes after a period $\Delta t$ seconds during which a constant action $u$, such that $\forall \tau \in \left[0, \Delta t\right]: u\in U(x(t + \tau))$, was being executed, we must integrate the velocity over time:

\[
x(t+\Delta t)=x(t) + \int_{0}^{\Delta t} f\left(x(t+\tau),u \right) d\tau.
\]

\todo{I must say something about sampling methods here.}
...
Our state space $X$ will consist of all the possible configurations of the vehicle. The state space will be slightly different based on the vehicle model which is used in order to capture all the variables required by the model including the configuration of the chassis and the state of the actuators.
...
\todo{Write collision detection section}
...
We will redefine the state transition function to integrate the velocity in the state space over a constant time period $\Delta t$ with a constant action $u\in U(x(t))$:
