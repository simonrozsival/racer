...
We will integrate state transition function to integrate the velocity in the state space over a constant time period $\Delta t$ with a constant action $u\in U(x(t))$ based on (\ref{eq:integrate_state_trajectory}) and we will call the resulting function $f_{\Delta t}$ the \textit{system simulator}:
...
\begin{defn}[Waypoint]
	...
	\[
		G_r(\vec{w})=\left\{x\in X \mid \norm{\vec{p_x}-\vec{w}}<r \land (\vec{p_x}, \vec{w}) \in D \right\},
	\]
	where $p_x$ is the position vector of the state $x$ and $r\in \mathbb{R}$ is the radius of the goal region. $G_r(\vec{w})$ is a set of all states which are considered to \textit{pass the waypoint}.
\end{defn}
...
\begin{defn}
	A sequence of states $\langle x_0,x_1,x_2,\ldots,x_k\rangle$ passes a sequence of waypoints $\langle \vec{w_1}, \vec{w_2}, \ldots, \vec{w_l} \rangle, l > 0$ when a sequence of indexes $\langle i_1,i_2,i_3, \ldots i_l \rangle$ exists such that:

	\begin{equation*}
		\begin{aligned}
			\forall m,n &\in \left\{1,\ldots,l\right\}: m<n\implies i_m < i_n \\
			\forall m &\in \left\{1,\ldots,l\right\}: x_{i_m}\in G_r(\vec{w_m}) \\
			i_l &= k \\
		\end{aligned}
	\end{equation*}
\end{defn}
...
\begin{defn}
	The goal condition $g_{\hat{w_l}}: X^* \rightarrow \left\{T,F\right\}$ is a function sutch that for a given sequence of waypoints $\hat{w_l}$ and a state trajectory  $\hat{x_k}=\langle x_0,x_1,x_2,\ldots, x_k\rangle, k>0: g_{\hat{w_l}}(\hat{x_k}) = T \iff \hat{x_k} \text{ passes } \hat{w_l}$.
\end{defn}
...
\begin{defn}[Trajectory Planning Problem]
	...
	\begin{itemize}
		\item A non-empty continuous set $X\subseteq\mathbb{R}^n$, which is the state space of the vehicle. Each state of the vehicle is represented by $n\in\mathbb{N}:n\geq3$ state variables and it contains the configuration of the vehicle in a two dimensional plane and other state variables as defined by the vehicle model which is used.
		...
		\item A goal condition function $g_{\hat{w}}: X^* \rightarrow \left\{ T, F\right\}$ for some fixed sequence of waypoints $\hat{w}=\langle w_0, w_1, w_2,\ldots, w_k \rangle, k>0$.
	\end{itemize}
\end{defn}
