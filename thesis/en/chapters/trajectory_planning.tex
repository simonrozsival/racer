...
\subsubsection{Brushed DC Motor Model}
...
The torque the motor produces can be modeled as an inverse proportion to the angular speed of the motor shaft. At the stall point $\tau_{s}$, the torque of the motor is the highest, but the motor does not spin. At the highest \gls{RPM} of the motor, the torque is zero. This speed is called the no load speed $\omega_n$. The torque to speed curve is shown in Figure~\ref{fig:torque_to_speed}\todo{Add the figure} and the immediate torque can be calculated from this formula:

\begin{equation*}
	\tau=\tau_s - \dfrac{\tau_s}{\omega_n}\omega,
\end{equation*}

where $\omega$ is the angular speed of the motor shaft. Our vehicle does not have a clutch or a torque convertor. This means that our motor is always physically connected to the wheels and the angular velocity of the wheels $\omega_w$ is directly proportional to the angular velocity of the engine $\omega$ and it is only scaled by the combined gear ratio of the gearbox and the differential $GR$. Our gearbox is fixed and so the gear ratio does not change and is constant. The wheel angular velocity is therefore expressed as:

\begin{equation*}
\omega_w = (GR) \omega.
\end{equation*}

The motor is controlled via an \gls{ESC}. We can set a specific throttle percentage and the \gls*{ESC} will increase or decrease the input voltage which will change the speed of the shaft rotation.

\todo[inline]{Finish this section by describing how the engine speed is adjusted from the data we collected.}
...
The name of the bicycle model, sometimes also referred to as one-track or single-track model in literature, comes from the fact, that we combine the effects of the two front wheels and the two rear wheels into a single virtual wheel located at the centers of the axles. This simplification makes reasoning about the movement of the vehicle easier while still being reasonably accurate, and it is commonly used in research \todo{multiple citations}. Our kinematic model is based on a model described by Dr. Rajesh Rajamani \cite{Rajamani_lateral_dynamics} and it is visualized in Figure~\ref{fig:kinematic_bicycle}.
...
\todo[inline]{This section is an early WIP and should not be reviewed just yet.}

\begin{equation*}
\begin{aligned}
F_{aero}&=\dfrac{1}{2}C_\alpha \rho A v^2=c_\alpha v^2 \\
...
\begin{equation*}
\begin{aligned}
F_{tr}&=F_{xr}+F_{xf}\cos\delta-F_{yf}\sin\delta \\
...
\dot{x}&=v\cos\left(\theta + \beta\right) \\
\dot{y}&=v\sin\left(\theta + \beta\right) \\
\dot{\beta}&=\dfrac{1}{mv}\left(F_{yf}+F_{yr}\right)-\dot{\theta} \\
...
\begin{figure}[h!]
	...
	\draw[vec] (cog) -- ++(2.25, 0) node[left, xshift=-8mm, yshift=-4.5mm] {$F_{tr}$};
...
For the sake of simplicity, our tire model is based on a simple linear tire model \todo{Add citation}. There are several other more advanced tire models, such as Brush, Fiala, or Pacejka models \todo{Add citations}, which give more precise results, especially for large slip angles. These models are not practical for us as they have many parameters which need to be experimentally identified.
...
From the slip angle, we can calculate the lateral force acting on each of the virtual wheels based on the slip angle curve. We approximate this curve with a simple linear model shown in \todo{Figure X} and expressed by the equation:
...
The traction maximum traction force of a virtual tire is:
\begin{equation*}
...
\begin{equation*}
X=\left\{\left(x,y,\theta,\dot{\theta},\beta,\omega,\delta\right)\mid x,y,\omega,\dot{\theta}\in\mathbb{R}; \theta,\delta,\beta\in\left[0, 2\pi\right)\right\}.
\end{equation*}
