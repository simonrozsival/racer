...
\begin{algorithm}[]
	...
	\KwIn{Trajectory planning problem $(X, U_f, c, f_{\Delta t}, x_0, X_g)$, maximum trajectory cost limit $g_{max}$, tie breaking function $\tau: X\rightarrow \mathbb{R}$}
	\KwOut{State trajectory or $null$}
	...
	\algorithm{\HybridAStar{$\delta$}}{
		$O\gets\left\{x_0\right\}, C\gets\emptyset, E\gets\emptyset$\;	
		\While{$O\neq\emptyset$}{
			$x\gets\argmin_{x'\in O} \Sigma(x')$\;
			$O\gets O\setminus\left\{x\right\}$\;
			$C\gets C\cup\left\{\delta(x)\right\}$\;
			\lIf{$x\in X_g$}{ \KwRet{\ReconstructTrajectory{$E$, $x$}} }
			\lIf{$g(x) < g_{max}$}{ \Expand{$x$, $O$, $C$, $E$, $\delta$} }
		}	
		\KwRet{null}\;
	}
	...
	\procedure{\Expand{$x$, $O$, $C$, $E$, $\delta$}}{
		\ForEach{$u\in U_f$}{
			$\left(x', k\right)\gets$ \Unfold{$x$, $u$, $\delta$}\;
			\If{$x' \neq \text{null} \land \delta(x')\notin C$}{
				\eIf{$\exists x''\in O: \delta(x')=\delta(x'')$}{
...
In this section we will analyze the pseudo-code shown in Algorithm~\ref{alg:hybrid_astar}. We will explain how it differs from the well-known A* algorithm and we will then also analyze the time and memory complexity of the algorithm.
