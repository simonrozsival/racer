...
\subsection{Clustering of the Pivot Points}

\todo[inline]{Describe the algorithm in general.}
...
\begin{algorithm}[]
	...
	\algorithm{\MergeClosePoints{$\hat{p}$, $d_{min}$}}{
		\Repeat{$n_{max} = 0$}{
			\Comment{Find the point with most close neighbors.}
			$i_{max}\gets 0$\;
			$n_{max}\gets 0$\;
			...
			\Comment{Merge the points in the biggest cluster.}
			\If{$n_{max}>0$}{
...
The algorithm stops, which is summarized as pseudocode in Algorithm~\ref{alg:merge_close_points}, if during an iteration no two consecutive points which are closer than $d_{min}$ are found in $\hat{p}$. If at least one cluster of points is merged, then at least two points from the original sequence are removed and only one point is added. In every iteration the length of the sequence therefore decreases. After at most $k-1=|\hat{p}|-1$ iterations, the number of points in the sequence will drop to a single point, and the algorithm will stop.

In every iteration, we find the biggest cluster of points along the sequence. We do this by counting the number of points within the radius $d_{min}$ around each point in the sequence. We pick the first point with this number of close neighbors and these neighbors. These points for a consecutive sub-sequence of the original sequence $\hat{p}$. We remove all of these points, except for the point in the middle of this sub-sequence. The point, which had most close neighbors and the point in the middle, can be two different points. This scenario is shown in Figure~\ref{fig:merge points}.
...
When the algorithm stops, we are guaranteed that now two consequent points are closer than $d_{max}$. If it was not the case, the algorithm would not stop.

\todo[inline]{Analyze the case when it collapses to just 1 point in the end.}

\paragraph{Time Complexity}

As we discussed earlier in this section, the outer loop will run at most $k-1=|\hat{p}|$ times. In each iteration, we first find the point 

\todo[inline]{Write how this could be improved.}

\paragraph{Memory Complexity}
\todo[inline]{Summarize the time complexity.}
