...
\todo[inline]{Mention that we chose this over simply splitting it into uniformly long segments.}
...
\todo[inline]{Add filtering by angle.}

\todo[inline]{Can I make the pseudocode simperl?}
...
\begin{algorithm}[]
	...
	\algorithm{\MergeClosePoints{$\hat{p}$, $d_{min}$}}{
		...
		\Repeat{$\hat{p}'\neq\hat{p}$}{
			...
			$i_{corner}\gets \argmin_{i\in N} $ \AngleAt{$i$, $\hat{p}$}\;
	...
	\procedure{\CloseNeighbors{$i$, $\hat{p}$, $d_{min}$}}{
		$k\gets |\hat{p}|$\;
		$N_{ahead}\gets $ \Walk{$i$, $\hat{p}$, $d_{min}$, $i\mapsto i+k-1\mod k$}\;
		$N_{after}\gets $ \Walk{$i$, $\hat{p}$, $d_{min}$, $i\mapsto i+1\mod k$}\;
	...
	\procedure{\AngleAt{$i$, $\hat{p}$}}{
		$k\gets |\hat{p}|, i_{prev}\gets i+k-1\mod k, i_{next}\gets i+1\mod k$\;
		$\vec{x}\gets \vec{p_{i_{prev}}} - \vec{p_{i}}$\;
		$\vec{y}\gets \vec{p_{i_{next}}} - \vec{p_{i}}$\;
		\KwRet{$\arccos\left(\dfrac{\vec{x}\cdot\vec{y}}{|\vec{x}||\vec{y}|}\right)$}\;
	}
...
\todo[inline]{The algorithm changed, rewrite the analysis.}
