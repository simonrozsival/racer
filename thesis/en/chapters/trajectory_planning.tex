...
The complete \textit{state space} $X$ of the vehicle will be a combination of the configuration space $C$, the number of passed waypoints, and several additional variables representing the kinematics and dynamics required by the differential constraints.
...
The \textit{system simulator} function describes the motion of the vehicle, but it does not model how the discrete state dimension of the number of passed waypoints $\mu$ behaves. We must extend the function $f_{\Delta t}$ to increase the number of passed waypoints when the next waypoint region is entered:
...
\begin{algorithm}
	...
	\procedure{\Unfold{$x_{start}$, $u$, $d$}}{		
		...
		\Repeat{$x_{last}=x_{prev}\lor d(x_{start})\neq d(x_{last})$}{
			...
			\lIf{$c(x_{last})=1$}{\KwRet{$\left(null, -1\right)$}}
			$k\gets k+1$\;
		}
...
\begin{algorithm}
	...
	\SetKwFunction{FindCenterline}{FindCenterline}
	\SetKwFunction{SelectSubsequence}{SelectSubsequence}
	\SetKwFunction{CorrectTailToHeadConnection}{CorrectTailToHeadConnection}
	
	\KwIn{Occupancy grid $O_r^{m\times n}$, initial state $x_0$, directly visible points relation $D$, sequence of checkpoints $\hat{p}_k=\left\langle \vec{p_1}, \vec{p_2}, \ldots, \vec{p_k} \right\rangle$}
	...
	\algorithm{\FindPivotPoints{$O_r^{m\times n}$, $x_0$, $\hat{p}_k$, $r_{track}$}}{
		$\hat{q}\gets $ \FindCenterline{$O_r^{m\times n}$, $x_0$, $\hat{p}_k$, $r_{track}$}\;
		$W\gets \emptyset$\;
		$i_{last}\gets 1$\;
		\ForEach{$i\in \left\{2,3,\ldots,|\hat{q}|\right\}$} {
			$(W, i_{last})\gets $ \ProcessNext{$W$, $i_{last}$, $i$}\;
		}
		\KwRet{\SelectSubsequence{$\hat{q}$, $W$}}\;
	}

	\BlankLine

	\procedure{\FindCenterline{$O_r^{m\times n}$, $x_0$, $\hat{p}_k$, $r_{track}$}}{
		\KwRet{\SpaceExploration{$O_r^{m\times n}$, $\kappa_{x,y}(x_0)$, $\hat{p}_k$, $\left[0.6 r_{track}, 2 r_{track}\right]$}}
	}
	...
	\procedure{\SelectSubsequence{$\hat{q}$, $I$}}{
		$\hat{s} \gets \langle \rangle$\;
		\ForEach{$i\in \left\{1,2,3,\ldots,|\hat{q}|\right\}$} {
			\If{$i\in I$}{
				$\hat{s}\gets \hat{s} + \langle \hat{q}_i \rangle$\;
			}
		}
		\KwRet{$\hat{s}$}\;
	}
...
\paragraph{No Obstacles Edge Case} In the case, when there are no obstacles in the occupancy grid, all of the points will be visible from each other and the set $W$ will be empty and consequently the list of pivot points would be empty. We can ignore this edge case and assume that the racing circuit has a non-trivial layout with at least two corners (a narrow oval).
...
\begin{algorithm}
	...
	\SetKwFunction{Neighbourhood}{Neighbourhood}
	...
	\algorithm{\FindCorners{$\hat{p}$, $\alpha_{max}$, $d_{min}$}}{
		$R\gets \left\{i\in\left\{1,\ldots,|\hat{p}|\right\}\mid \alpha(\vec{p_{i-1}}-\vec{p_i},\vec{p_{i+1}}-\vec{p_i})\leq\alpha_{max}\right\}$\;
		$\hat{p}'\gets $ \SelectSubsequence{$\hat{p}$, $R$}\;
	...
	\procedure{\MergeClosePoints{$\hat{p}$, $d_{min}$}}{
		...
		\Repeat{$\hat{p}'\neq\hat{p}$}{
			...
			$i_{max}\gets \argmax_{i\in \left\{1,\ldots,k\right\}} |$\Neighbourhood{$i$, $\hat{p}$, $d_{min}$}$|$\;
			$N\gets $ \Neighbourhood{$i_{max}$, $\hat{p}$, $d_{min}$}\;
			$i_{corner}\gets \argmin_{i\in N} \alpha(\vec{p_{i-1}}-\vec{p_i}, \vec{p_{i+1}}-\vec{p_i})$\;
			$\hat{p}'\gets $ \SelectSubsequence{$\hat{p}$, $\left\{1,\ldots,k\right\}\setminus N\cup\left\{i_{corner}\right\}$}\;
		}
		\KwRet{$\hat{p}$}\;
	}
...
At the beginning of the algorithm, we iterate over all of the $k$ pivot points and remove the points with large obtuse angles. Calculating the angle is a constant operation and so this part requires $O(k)$ time.
