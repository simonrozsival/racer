...
Hector SLAM provides a \gls*{ROS} node \verb|hector_mapping| which subscribes to a topic providing \verb|sensor_msgs/LaserScan| messages and publishes its output in a topic of type \verb|nav_msgs/OccupancyGrid|\footnote{http://docs.ros.org/melodic/api/nav\_msgs/html/msg/OccupancyGrid.html}. An important parameter is the resolution of the occupancy grid which we chose to be \SI{2}{\centi\meter}. When the mapping is complete, the final map can be saved into a file using a \verb|map_saver| node from the \verb|map_server| package\footnote{http://wiki.ros.org/map\_server\#map\_saver}.

We prepared a \gls*{ROS} launch file with the configuration of all nodes necessary for this task. This configuration is included in the attached files in \newline\verb|/racer-jetson/catkin_ws/src/racer/launch/create_map.launch|. The proper usage of this launch file is described in Chapter~\ref{chapter:user_documentation}.

\subsection{Racing}

This is significantly more complicated than the mapping task and the implementation is split into three groups of \gls*{ROS} nodes: odometry and localization, agent behavior, and a hardware interface.

We prepared a \gls*{ROS} launch file with the configuration of all nodes necessary for the racing task in the attached files in \newline\verb|/racer-jetson/catkin_ws/src/racer/launch/race.launch|. This launch file requires two arguments: a map definition file and a circuit definition file. The proper usage of this launch file is described in Chapter~\ref{chapter:user_documentation}.

\subsubsection{Track Map}
...
\Gls{ROS} includes a very useful way of keeping track of relative position of the robot in multiple coordinate frames in the form of a package called \verb|tf|\footnote{http://wiki.ros.org/tf}. \verb|tf| maintains a tree structure of coordinate frames and it keeps track of transformations between these coordinate frames as they change over time. The updates to the transformations are shared through a regular \gls*{ROS} topic called \verb|tf|. The \verb|tf| package provides libraries for both Python and C++ which make keeping track of the current state and manipulating with the linear transformations easy for the programmer.
 
Our implementation adheres to the standard naming convention of coordinate frames as it is defined in \gls{REP} 105\footnote{https://www.ros.org/reps/rep-0105.html}. The root (``fixed'') frame is called \verb|map| and it is aligned with the origin of the map. The \verb|map| frame has a child frame \verb|odom|. The transformation between these two coordinate frames represents the correction of odometry drift and it is published by the \verb|amcl| \gls*{ROS} node. The child frame of \verb|odom| is called \verb|base_link| and it represents the local coordinate frame of the vehicle with the $x$ axis aligned in the heading direction of the vehicle. The transformation between \verb|odom| and \verb|base_link| is published by the \verb|robot_pose_ekf| node which fuses odometry sources into one position estimate.

The sensors, which are attached to the body of the vehicle, have their own coordinate frames which are not perfectly aligned with \verb|base_link|. We use the \verb|static_transform_publisher| node from the \verb|tf| package to ensure correct transformation between the sensors and the center of gravity of the vehicle. Namely, the \gls*{IMU} required a rotation of \ang{90} around the $z$ axis and the \gls*{LIDAR} was moved several centimeters in the positive direction along the $x$ axis of \verb|base_link|. We assume that our vehicle always moves only on a flat surface and so we do not require any transformations along the $z$ axis which is perpendicular to the road surface. The \verb|tf| tree for the racing setup is shown in Figure~\ref{fig:tf_tree_racing}.
...
\paragraph{DWA Following Node}

This node finds the best control input for the actuators in order to follow the planned trajectory based on the current state of the vehicle using the \gls{DWA} algorithm.
