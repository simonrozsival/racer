...
int main(int argc, char **argv)
{
  ...
  double wheelbase, rear_wheel_radius_degrees, max_steering_angle;
  
  nh.param<double>("wheelbase", wheelbase, 0.3);
  nh.param<double>("rear_wheel_radius", rear_wheel_radius_degrees, 0.1);
  nh.param<double>("max_steering_angle", max_steering_angle, 30);
  
  double rear_wheel_radius = rear_wheel_radius_degrees / 180 * M_PI;
  ...
  nh.param<std::string>("driving_topic", driving_topic, "/racer/driving/commands");
  ...
  ros::Publisher odometry_pub = nh.advertise<nav_msgs::Odometry>(odometry_topic, 1, true);
