...
int main(int argc, char *argv[])
{
  ...
  std::string state_topic, racing_line_topic, ackermann_commands_topic, twist_commands_topic;
  ...
  node.param<std::string>("ackermann_commands_topic", ackermann_commands_topic, "/racer/ackermann_commands");
  node.param<std::string>("tiwst_commands_topic", twist_commands_topic, "/racer/commands");
  ...
  auto state_sub = node.subscribe<racer_msgs::State>(state_topic, 1, &state_update);
  auto racing_line_sub = node.subscribe<racer_msgs::RacingLine>(racing_line_topic, 1, &racing_line_update);

  auto ackermann_pub = node.advertise<ackermann_msgs::AckermannDrive>(ackermann_commands_topic, 1);
  auto twist_pub = node.advertise<geometry_msgs::Twist>(twist_commands_topic, 1);

  auto nearest_point_pub = node.advertise<geometry_msgs::PoseStamped>(nearest_point_topic, 1);
  ...
  while (ros::ok())
  {
    if (racing_line && last_known_state)
    {
      ...
      const auto action = model->action_from_speed_and_steering_angle(speed, angle);

      ackermann_pub.publish(racer_ros::action_to_ackermann_msg(action, *vehicle->steering_servo));
      twist_pub.publish(racer_ros::action_to_twist_msg(action));

      // Visualization
      nearest_point_pub.publish(racer_ros::pose_msg_from_point(racing_line->spline()[closest_pt_index].coordinate));
