...
void waypoints_update(const racer_msgs::Waypoints::ConstPtr &waypoints) {
  ...
  auto cell_size = 2 * model->chassis->radius();
  auto discretization =
      std::make_unique<racer::astar::hybrid_astar::discretization>(
          cell_size, cell_size, 2 * M_PI / 32.0, 764.0 / 30.0);
...
int main(int argc, char *argv[]) {
  ...
  std::string map_topic, state_topic, trajectory_topic, path_topic,
      waypoints_topic, inflated_map_topic, visualization_topic;
  ...
  ros::Publisher visualization_pub =
      node.advertise<visualization_msgs::MarkerArray>(visualization_topic, 1);
  ...
  const auto actions = racer::action::create_actions(
      throttle_levels, steering_levels, min_throttle, max_throttle);
  ...
  while (ros::ok()) {
    if (planner && last_known_state.is_valid() && !next_waypoints.empty()) {
      ...
      const auto planning_result =
          planner->plan(last_known_state, actions, circuit, collision_detector,
                        next_waypoint);
      ...
      const auto elapsed_time =
          std::chrono::duration_cast<std::chrono::milliseconds>(end_clock -
                                                                start_clock);
      // visualization_pub.publish(planning_result.visualization);

      if (!planning_result.trajectory) {
        ...
        trajectory_pub.publish(*planning_result.trajectory);
        normal_rate.sleep();
      }
    }
