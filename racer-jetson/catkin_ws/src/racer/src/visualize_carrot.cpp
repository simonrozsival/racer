...
std::optional<racer::trajectory<racer::vehicle_model::kinematic::state>>
    trajectory;
...
int main(int argc, char *argv[]) {
  ...
  auto waypoints_sub = node.subscribe<racer_msgs::Waypoints>(
      waypoints_topic, 1, waypoints_observed);
  ...
  racer::following_strategies::target_locator<
      ...
      target_locator{min_lookahead, max_lookahead, 764.0};
  ...
  while (ros::ok()) {
    if (state && trajectory) {
      const auto target =
          target_locator.find_target(*state, next_waypoint, *trajectory);
