...
void state_update(const racer_msgs::State::ConstPtr msg) {
  ...
  racer::vehicle_configuration current_configuration{msg->x, msg->y,
                                                     msg->heading_angle};

  // the circuit is defined by the initial (current) configuration of the
  // vehicle and the given checkpoints along the track
  std::vector<racer::math::point> final_check_points{check_points.begin(),
                                                     check_points.end()};
  final_check_points.push_back(
      current_configuration.location()); // back to the start

  // calculate the centerline of the track in the occupancy grid and detect the
  // corners along the centerline
  const auto centerline = racer::track::centerline::find(
      current_configuration, grid, final_check_points);
  ...
  racing_line = racer::track::racing_line::basic_from_apexes(grid, corners,
                                                             maximum_speed);
  new_line = true;
}
...
int main(int argc, char *argv[]) {
  ...
  node.param<std::vector<std::string>>("check_points", checkpoint_params,
  ...
  ros::Publisher racing_line_pub =
      node.advertise<racer_msgs::RacingLine>(racing_line_topic, 1, true);
