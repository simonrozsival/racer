...
namespace astar {    
    ...
    template <typename TDiscreteState>
    class discretized_search_problem : public astar::base_search_problem<TDiscreteState, state, trajectory> {
    public:
        discretized_search_problem(
            ...
            : astar::base_search_problem<TDiscreteState, state, trajectory>(std::move(initial_state), std::move(discrete_state)),
            time_step_s_(time_step_s),
            vehicle_(vehicle),
            transition_(std::make_unique<math::euler_method>(time_step_s), vehicle),
    ...
    public:
        virtual std::list<astar::neighbor_transition<TDiscreteState, state>> valid_neighbors(const state& examined_state) const override {
            std::list<astar::neighbor_transition<TDiscreteState, state>> transitions;
            ...
            for (const auto& action : available_actions_) {
                ...
                states.push_back(std::move(prediction));
                transitions.emplace_back(astar::neighbor_transition<TDiscreteState, state>(
