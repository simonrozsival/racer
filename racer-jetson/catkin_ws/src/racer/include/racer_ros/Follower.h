...
template <typename State> class Follower {
public:
  Follower(
      std::shared_ptr<racer::occupancy_grid> map,
      std::shared_ptr<racer::following_strategies::following_strategy<State>>
          strategy,
      const double time_step_s)
      : map_{map}, strategy_{strategy}, stop_{0, 0}, time_step_s_{time_step_s},
        next_waypoint_{0}, state_{} {}
  ...
  void state_observed(const racer_msgs::State::ConstPtr &state) {
    racer::vehicle_configuration position = {state->x, state->y,
                                             state->heading_angle};
  ...
  racer::action select_driving_command() const {
    if (reference_trajectory_.is_valid()) {
      return strategy_->select_action(state_, next_waypoint_,
                                      reference_trajectory_, map_);
    } else {
  ...
  const State &last_known_state() const { return state_; }
...
private:
  const std::shared_ptr<racer::following_strategies::following_strategy<State>>
      strategy_;
