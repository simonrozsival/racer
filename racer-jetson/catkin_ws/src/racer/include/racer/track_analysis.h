    ...
    const double vehicle_radius)
{
    ...
    const auto circle_path = exploration.explore_grid(inflated_grid, initial_configuration, checkpoints);
    if (circle_path.empty())
    {
        return {};
    }

    auto waypoints = analysis.find_corners(analysis.find_pivot_points(circle_path, inflated_grid), max_angle);
