...
template <typename TKey, typename State> struct neighbor_transition {
  neighbor_transition(TKey key, std::vector<State> states,
                      racer::action previous_action, const double cost)
...
template <typename TKey, typename State> struct search_node {
  const TKey key;
  const std::vector<State> states;
  const racer::action previous_action;
  ...
  search_node(TKey key, std::vector<State> states,
              racer::action previous_action,
              std::weak_ptr<search_node<TKey, State>> parent,
              double cost_to_come, double cost_estimate,
              std::size_t passed_waypoints)
      : key{key}, states{states}, previous_action{previous_action},
        cost_to_come{cost_to_come}, cost_estimate{cost_estimate},
        passed_waypoints{passed_waypoints}, parent{parent} {}

  static auto for_initial_state(TKey key, State state) {
    return std::make_unique<search_node<TKey, State>>(
        key, std::vector<State>{state}, action{0, 0},
  ...
  inline State final_state() const { return states.back(); }
};

template <typename TKey, typename State> class base_search_problem {
public:
  virtual ~base_search_problem() = default;
  virtual std::vector<neighbor_transition<TKey, State>>
  valid_neighbors(const search_node<TKey, State> &node) const = 0;
  virtual const bool is_goal(std::size_t passed_waypoints) const = 0;
  virtual const bool passes_waypoint(const std::vector<State> &examined_state,
                                     size_t passed_waypoints) const = 0;
  virtual const double
  estimate_cost_to_go(const State &state,
                      std::size_t passed_waypoints) const = 0;
  virtual const trajectory<State>
  reconstruct_trajectory(const search_node<TKey, State> &node) const = 0;
  virtual std::unique_ptr<search_node<TKey, State>>
  initial_search_node() const = 0;
};

template <typename TKey, typename State> struct search_node_comparator {
...
template <typename TKey, typename State> class closed_set {
private:
  std::unordered_set<std::pair<TKey, size_t>> data_;

public:
  bool contains(const TKey &key, const size_t passed_waypoints) const {
    return data_.find(std::pair<TKey, size_t>(key, passed_waypoints)) !=
           data_.end();
  }
...
template <typename TKey, typename State> class open_set {
private:
  std::priority_queue<std::shared_ptr<search_node<TKey, State>>,
                      std::vector<std::shared_ptr<search_node<TKey, State>>>,
...
public:
  ...
  bool is_empty() const { return data_.size() == 0; }
...
template <typename State> struct search_result {
  ...
  search_result(std::size_t number_of_opened_nodes,
                std::vector<State> expanded_nodes)
      : found_trajectory{}, number_of_opened_nodes{number_of_opened_nodes},
        expanded_nodes{expanded_nodes}, final_cost{0} {}

  search_result(racer::trajectory<State> found_trajectory,
                std::size_t number_of_opened_nodes,
                std::vector<State> expanded_nodes, double final_cost)
      : found_trajectory{found_trajectory},
        number_of_opened_nodes{number_of_opened_nodes},
        expanded_nodes{expanded_nodes}, final_cost{final_cost} {}
  ...
  inline bool was_successful() const { return found_trajectory.is_valid(); }
};

template <typename TKey, typename State>
const search_result<State>
search(std::shared_ptr<base_search_problem<TKey, State>> problem,
       const std::atomic<bool> &terminate) {
  open_set<TKey, State> opened_nodes{problem->initial_search_node()};
  closed_set<TKey, State> closed_nodes;
  std::vector<std::shared_ptr<search_node<TKey, State>>>
      expanded_nodes; // we must remember the whole graph so that we
                      // can reconstruct the final path from the
                      // weak pointers
  std::vector<State> explored_states; // we must remember the whole graph so
                                      // that we can reconstruct the final path
  ...
  while (!terminate && !opened_nodes.is_empty()) {
    ...
    explored_states.push_back(expanded_node->states.front());
    ...
    if (problem->is_goal(expanded_node->passed_waypoints)) {
      return {problem->reconstruct_trajectory(*expanded_node),
              ...
              explored_states, expanded_node->cost_to_come};
    }
    ...
    for (auto neighbor : problem->valid_neighbors(*expanded_node)) {
      size_t passed_waypoints =
          problem->passes_waypoint(neighbor.states,
      ...
      opened_nodes.push(std::make_unique<search_node<TKey, State>>(
          neighbor.key, neighbor.states, neighbor.previous_action,
          expanded_node, cost_to_come,
          cost_to_come + problem->estimate_cost_to_go(neighbor.final_state(),
                                                      passed_waypoints),
          passed_waypoints));
    }
  }

  return {opened_nodes.number_of_opened_nodes_since_initial_state(),
          explored_states};
};
