...
struct vehicle_chassis
{
  ...
  vehicle_chassis(double cog_offset, double wheelbase, double width, double length, double wheel_radius,
                  double motor_to_wheel_gear_ratio, std::unique_ptr<steering_servo_model> steering_servo,
                  std::unique_ptr<motor_model> motor)
      : distance_of_center_of_gravity_to_rear_axle(cog_offset), wheelbase(wheelbase), width(width), length(length), wheel_radius(wheel_radius), motor_to_wheel_gear_ratio(motor_to_wheel_gear_ratio), steering_servo(std::move(steering_servo)), motor(std::move(motor))
  {
  }
  ...
  {
    return std::make_unique<vehicle_chassis>(0.25, // cog_offset
                                             0.5,  // wheelbase
                                             0.30, // safe width
                                             0.45, // safe length
                                             0.05, // wheel radius
                                             6.0,  // wheel to motor ratio
