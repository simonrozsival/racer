...
public:
  steering_servo_model(racer::math::angle max_steering_angle_left,
      ...
      : max_steering_angle_left_{max_steering_angle_left},
        ...
        angle_to_pwm_{angle_to_pwm}, pwm_adjustment_coefficients_{
                                         pwm_adjustment_coefficients} {}

  static auto simulator() {
    return std::make_unique<steering_servo_model>(
        angle::from_degrees(-75), angle::from_degrees(75),
  ...
  static auto rc_beast() {
    return std::make_unique<steering_servo_model>(
        angle::from_degrees(-21.16), angle::from_degrees(26.57),
        std::array<double, 2>{12.701, 1476.686},
        std::array<double, 2>{0.000329, 0.1174});
  }
  ...
  angle predict_next_state(const angle &current_steering_angle,
    ...
    const angle angle_distance =
        std::min(target_angle - current_steering_angle,
                 2 * M_PI - (target_angle - current_steering_angle));
    const double angle_change_rate =
        angle_distance / time_to_adjust(current_steering_angle, target_angle);
  ...
  angle target_steering_angle(const racer::action &action) const {
    ...
    const angle left_or_right = target_steering_angle_percentage > 0
                                    ? max_steering_angle_right_
                                    : max_steering_angle_left_;
    return left_or_right * std::abs(target_steering_angle_percentage);
  }

  double necessary_action_target_angle(const angle &target) const {
    const angle max =
        target > 0 ? max_steering_angle_left_ : max_steering_angle_right_;
    return -std::abs(target) / max;
  }

  angle max_steering_angle() const { return max_steering_angle_right_; }
...
private:
  ...
  inline double time_to_adjust(const angle alpha, const angle beta) const {
    ...
    return pwm_adjustment_coefficients_[0] * pwm_distance +
           pwm_adjustment_coefficients_[1];
  }
};
