...
public:
  ...
  select_steering_angle(const State state,
    ...
    auto diff =
        target.location() - calculate_rear_axle_center(state.configuration());
    auto dist = target.location().distance(
        calculate_rear_axle_center(state.configuration()));
...
private:
  ...
  inline racer::math::point
  calculate_rear_axle_center(const racer::vehicle_configuration &cfg) const {
