...
public:
  ...
  racer::action select_action(
    ...
    const auto throttle =
        closest->state().motor_rpm() / current_state.motor_rpm() > 1.1
            ? 1.0
            : closest->state().motor_rpm() / model_->chassis->motor->max_rpm();

    const auto steering_input =
        model_->chassis->steering_servo->action_input_for_angle(steering_angle);
