...
public:
                         ...
                         const std::shared_ptr<racer::occupancy_grid> map) const
  {
    ...
    {
      total_error += calculate_error(*it_a, it_b->state(), map);
    }
...
private:
  ...
  double obstacle_proximity_error(const State &a, const std::shared_ptr<racer::occupancy_grid> grid) const
  {
    ...
    const double head_on_dist =
        grid->find_distance(a.position(), a.configuration().heading_angle(), ideal_head_on_distance);
    const double any_dir_dist = grid->distance_to_closest_obstacle(a.position(), ideal_min_distance);

    return std::max((ideal_head_on_distance - head_on_dist) / ideal_head_on_distance,
