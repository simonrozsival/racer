...
odometry_subject::odometry_subject(
    ...
    std::unique_ptr<racer::vehicle_model::base_model<racer::math::angle>>
        servo_model,
    std::unique_ptr<racer::vehicle_model::kinematic::model> vehicle_model,
    const std::string &odometry_frame, const std::string &base_link,
    tf::TransformBroadcaster &transform_broadcaster,
    ros::Publisher &odometry_topic, ros::Publisher &motor_rpm_topic)
    : configuration_{}, last_motor_update_time_{0}, total_revolutions_{0},
      shaft_to_motor_gear_ratio_{gear_ratio}, last_servo_update_time_{0},
      steering_angle_{0}, last_update_time_{0}, servo_model_{std::move(
                                                    servo_model)},
      vehicle_model_{std::move(vehicle_model)}, base_link_{base_link},
      odometry_frame_{odometry_frame},
      transform_broadcaster_{transform_broadcaster},
      odometry_topic_{odometry_topic}, motor_rpm_topic_{motor_rpm_topic} {
  last_servo_update_time_ = ros::Time::now().toSec();
  last_motor_update_time_ = ros::Time::now().toSec();
}

void odometry_subject::process_steering_command(
    const geometry_msgs::Twist::ConstPtr &msg) {
  const double t = ros::Time::now().toSec();
  const double dt = t - last_servo_update_time_;

  const auto action =
      racer::action{0, msg->angular.z}; // we don't care about the throttle
  steering_angle_ =
      servo_model_->predict_next_state(steering_angle_, action, dt);

  last_servo_update_time_ = t;
}

void odometry_subject::process_wheel_odometry(
...
void odometry_subject::publish_odometry(bool publish_tf) {
  ...
  const auto current_state = racer::vehicle_model::kinematic::state{
      configuration_, current_rpm_, steering_angle_};

  const auto prediction = vehicle_model_->predict_next_state(
      current_state, racer::action{0, 0}, elapsed_time);
  const auto angular_velocity =
      configuration_.heading_angle().distance_to(
          prediction.configuration().heading_angle()) /
      elapsed_time;
  ...
  if (publish_tf) {
    ...
    geometry_msgs::Quaternion odom_quat =
        tf::createQuaternionMsgFromYaw(configuration_.heading_angle());
...
void odometry_subject::publish_state_estimate(
  ...
  geometry_msgs::Quaternion odom_quat =
      tf::createQuaternionMsgFromYaw(predicted_configuration.heading_angle());
  ...
  odom.twist.twist.linear.x =
      vehicle_model_->calculate_speed_with_no_slip_assumption(current_rpm_);
