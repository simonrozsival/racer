...
int main(int argc, char **argv) {
  ...
  auto servo_model =
      racer::vehicle_model::hardware_steering_servo_model::with_fitted_values();
  auto vehicle = racer::vehicle_model::vehicle_chassis::rc_beast();
  auto vehicle_model = std::make_unique<racer::vehicle_model::kinematic::model>(
      std::move(vehicle));
  ...
  std::string driving_topic, wheel_encoders_topic, motor_rpm_topic,
      odometry_topic;

  nh.param<std::string>("driving_topic", driving_topic, "/racer/commands");
  nh.param<std::string>("wheel_encoders_topic", wheel_encoders_topic,
                        "/racer/wheel_encoders");
  ...
  ros::Publisher odometry_pub =
      nh.advertise<nav_msgs::Odometry>(odometry_topic, 10, true);
  ros::Publisher motor_rpm_pub =
      nh.advertise<nav_msgs::Odometry>(motor_rpm_topic, 10, true);
  tf::TransformBroadcaster odom_broadcaster;

  odometry_subject subject(gear_ratio, std::move(servo_model),
                           std::move(vehicle_model), odometry_frame, base_link,
  ...
  ros::Subscriber wheel_encoders_sub = nh.subscribe<std_msgs::Float64>(
      wheel_encoders_topic, 1, &odometry_subject::process_wheel_odometry,
      &subject);
